...
class HospitalViewSet(viewsets.ModelViewSet):
    ...
    def province_beds_capacity(self, request):
        province_beds_capacity = Hospital.objects.values("province_name").annotate(bed_capacity=Sum("beds"))
        return Response(province_beds_capacity)

    @action(detail=False, methods=["get"])
    def closest_hospitals(self, request):
        """Get Hospitals that are at least 3km or less from a user location"""
